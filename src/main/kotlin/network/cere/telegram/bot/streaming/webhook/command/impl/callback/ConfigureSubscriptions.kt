...
class ConfigureSubscriptions(
    ...
    override fun command() = "/configureSubscriptions"
    ...
    override fun handle(update: Update) {
        ...
        chatContext.entityName = ContextEntity.SUBSCRIPTION
        chatContext.modificationStep = ContextModificationStep.DURATION
        user.chatContextJson = json.encodeToString(chatContext)
        user.persistAndFlush()

        botProducer.sendTextMessage(
            message.chat.id,
            "Send me the length of subscription in days"
        )
    }
}